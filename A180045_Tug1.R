...
ggplot(over_cat2) + geom_area(aes(x = date, y = over_cat, group = category, fill = category)) + theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1))+ scale_x_date(date_breaks = "1 month", date_labels = "%B %Y") + guides(colour = guide_legend("Legend")) + ylab("CPI") + ggtitle("Overall CPI Trend for each Category") + scale_fill_viridis(discrete = T, option = "C") -> plot
ggplotly(plot)
